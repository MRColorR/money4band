...
def assemble_docker_compose(app_config_path_or_dict: Any, user_config_path_or_dict: Any, m4b_config_path_or_dict: Any, compose_output_path: str = './docker-compose.yaml') -> None:
    """
    Assemble a Docker Compose file based on the app and user configuration.

    Arguments:
    app_config_path_or_dict -- the path to the app configuration file or the configuration dictionary
    user_config_path_or_dict -- the path to the user configuration file or the configuration dictionary
    m4b_config_path_or_dict -- the path to the m4b configuration file or the configuration dictionary
    ...
    for app in app_config['apps']:
        ...
        if user_config['apps'][app_name]['enabled']:
            ...
            # Use environment variables directly without replacement
            services[app_name]['environment'] = [
                env for env in services[app_name]['environment']
            ]
...
def generate_env_file(m4b_config_path_or_dict: Any, user_config_path_or_dict: Any, env_output_path: str = './.env') -> None:
    """
