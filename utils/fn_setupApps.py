...
def configure_password(app: Dict, flag_config: Dict, config: Dict):
    ...
    password = ask_string(
        f'Enter your {app["name"].lower().title()} password:', default=config.get("password"))
...
def configure_apikey(app: Dict, flag_config: Dict, config: Dict):
    ...
    apikey = ask_string(
        f'Enter your {app["name"].lower().title()} APIKey:', default=config.get("apikey"))
...
def configure_userid(app: Dict, flag_config: Dict, config: Dict):
    ...
    userid = ask_string(
        f'Enter your {app["name"].lower().title()} UserID:', default=config.get("userid"))
...
def configure_cid(app: Dict, flag_config: Dict, config: Dict):
    print(
        f'Find your CID inside your {app["name"].lower().title()} dashboard/profile.')
...
def collect_user_info(user_config: Dict[str, Any], m4b_config: Dict[str, Any]) -> None:
    ...
    device_name = user_config['device_info'].get(
    ...
    if ask_question_yn(f'The current device name is "{device_name}". Do you want to change it?'):
        ...
        new_device_name = input(
            'Enter your new device name (or leave blank to generate one randomly): ').strip()
    ...
    for app in apps:
        ...
        if config.get('enabled'):
            ...
            if ask_question_yn('Do you want to disable it?'):
                ...
                continue
            print(
                f'Do you want to change the current {app_name} configuration?')
...
def validate_notification_url(url: str) -> bool:
    ...
    for service, pattern in SUPPORTED_NOTIFICATION_SERVICES.items():
        if re.match(pattern, url):
            logging.info(
                f"Given URL {url} matches notification service {service}")
            return True
    logging.error(
        f"Given URL {url} does not match any supported notification service")
    return False
...
def setup_notifications(user_config: Dict[str, Any]) -> None:
    ...
    if ask_question_yn('Do you want to enable notifications about apps images updates?'):
        logging.info(
            "User decided to set up notifications about apps images updates.")
        ...
        while True:
            notification_url = ask_string(
                'Enter the notification URL (e.g., discord://token@id):', default=notifications_config.get('url', ''))
...
def setup_multiproxy_instances(user_config: Dict[str, Any], app_config: Dict[str, Any], m4b_config: Dict[str, Any], proxies: list) -> None:
    ...
    base_subnet = ((pow(2, base_netmask) - 1) <<
                   (32 - base_netmask)) & base_subnet
    ...
    for i, proxy in enumerate(proxies):
        logging.info(
            f"Creating instance {i+1}/{len(proxies)} with proxy: {proxy}")
...
def main(app_config_path: str, m4b_config_path: str, user_config_path: str) -> None:
    ...
    try:
        ...
        if proxy_setup:
            ...
            write_json(user_config, user_config_path)
            assemble_docker_compose(m4b_config_path_or_dict=m4b_config, app_config_path_or_dict=app_config,
                                    user_config_path_or_dict=user_config, compose_output_path='./docker-compose.yaml', is_main_instance=True)
            generate_env_file(m4b_config_path_or_dict=m4b_config, app_config_path_or_dict=app_config,
                              user_config_path_or_dict=user_config, env_output_path='./.env', is_main_instance=True)

            setup_multiproxy_instances(
                user_config, app_config, m4b_config, proxies)
            ...
            generate_env_file(m4b_config_path, app_config_path, user_config_path,
                              env_output_path='./.env', is_main_instance=True)
        ...
        logging.error(
            f"An error occurred in main setup apps process: {str(e)}")
        raise
...
if __name__ == '__main__':
    ...
    parser.add_argument('--user-config-path', type=str,
                        default='./config/user-config.json', help='Path to user_config JSON file')
