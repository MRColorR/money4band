...
def mainmenu(m4b_config_path: str, apps_config_path: str, user_config_path: str, utils_dir_path: str) -> None:
    ...
    while True:
        try:
            ...
            logging.error(
                f"An error occurred while loading configurations: {str(e)}")
            raise
        ...
        try:
            ...
            logging.info(
                f"System info and default platform stored: {device_info}")
            ...
            m4b_tools_modules = loader.load_modules_from_directory(
                utils_dir_path)
            ...
            logging.error(
                f"An error occurred while setting up the menu: {str(e)}")
            raise
        try:
            ...
            logging.error(
                f"An error occurred while processing the menu: {str(e)}")
            raise
...
def main():
    ...
    parser.add_argument('--config-dir', default=os.path.join(script_dir,
                        'config'), help='Set the config directory')
    parser.add_argument('--config-m4b-file', default='m4b-config.json',
                        help='Set the m4b config file name')
    parser.add_argument('--config-usr-file', default='user-config.json',
                        help='Set the user config file name')
    parser.add_argument('--config-app-file', default='app-config.json',
                        help='Set the apps config file name')
    parser.add_argument('--utils-dir', default=os.path.join(script_dir,
                        'utils'), help='Set the m4b tools directory')
    parser.add_argument('--requirements-path', default=os.path.join(script_dir,
                        'requirements.toml'), help='Set the requirements path')
    ...
    parser.add_argument('--log-file', default='m4b.log',
                        help='Set the logging file name')
    parser.add_argument('--template-user-config-path', default=os.path.join(script_dir,
                        'template', 'user-config.json'), help='Set the template user config file path')
    ...
    rotating_handler = RotatingFileHandler(
        log_file_path, maxBytes=1*1024*1024, backupCount=3)  # 1 MB per file, keep 3 backups
    ...
    try:
        mainmenu(
            m4b_config_path=os.path.join(
                args.config_dir, args.config_m4b_file),
            apps_config_path=os.path.join(
                args.config_dir, args.config_app_file),
            user_config_path=user_config_path,
